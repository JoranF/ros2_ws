...
void CarGazeboPlugin::Load(gazebo::physics::ModelPtr model,
  ...
  // fake_car code here
  {
    ...
    // // subscribe
    // joy_sub = ros_node_->create_subscription<sensor_msgs::msg::Joy>("/joy_", 2, std::bind(&CarGazeboPlugin::joy_callback, this, std::placeholders::_1));
...
void CarGazeboPlugin::Update() {
  ...
  if (update_dt * 1000 >= update_period_ms_) {
    ...
    auto alpha = 0.3;
    steering_angle = alpha * steering_angle + (1 - alpha) * pre_steer;
    engine_speed = alpha * engine_speed + (1 - alpha) * pre_rpm;
    pre_steer = steering_angle;
    pre_rpm = engine_speed;
    
    smartcar_msgs::msg::Status status_msg;
    status_msg.steering_angle_rad = steering_angle;
    status_msg.engine_speed_rpm = engine_speed;
